        ...
        <AnimatePresence> {toastMessage &&
          <motion.div variants={variants} animate={"show"} initial={"hide"} exit={"hide"}>
            <div className="relative flex justify-between py-4 bg-white rounded-lg h-fit shadow-lg shadow-gray-200 border text-sm text-slate-600 items-center pointer-events-auto mb-12">
              ...
              <button className="absolute right-0 mx-3 p-1 cursor-pointer hover:opacity-50 rounded-full transition-all" onClick={() => toastMessageSet("")}>
                {/* [Credit]: svg from https://heroicons.dev */}
                <svg className="h-4" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
